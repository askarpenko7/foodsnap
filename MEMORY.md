...
- **Both classifiers are generic, and this shapes the UX.** ML Kit sees a margherita pizza as `Food 96%`, `Pizza 95%`, `Cuisine 90%`, `Cake 78%`. Vision is worse: on a salad it returns `tableware 49%`, `utensil 49%`, `bowl 49%` *above* `food` and `salad` — it ranks the objects over the meal. `src/lib/labels.ts` sorts labels into tiers (food → category → object) and orders by confidence only *within* a tier, so a 4% "lettuce" beats a 49% "tableware". **Add to those sets from observed output, not imagination**, and pin the real output as a test fixture. Wrong-but-specific guesses like "Cake" stay bright — the user corrects those, not a heuristic.
- **Vision returns ~1300 labels and spreads confidence thinly across all of them.** This is the single most misleading thing about it. A top-5/0.1-floor window — the obvious choice, and what ML Kit's numbers suggest — throws the answer away before JS ever sees it: on a real salad photo the entire top five was crockery, and lettuce (4.7%), tomato (4.1%) and cucumber (2.2%) all sat under the floor. iOS therefore hands over **20 candidates from a 0.01 floor** and lets the shared JS ranking choose. **A low Vision confidence does not mean a bad label** — filter after ranking, never before. Android is deliberately left at 5/0.1 because ML Kit applies its own 0.5 threshold internally and returns few, well-separated labels; the asymmetry is intentional and commented in `FoodClassifierModule.kt`.
